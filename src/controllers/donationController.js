...
exports.initializePayment = async (req, res) => {
  try {
    const { project, amount, phone, paymentMethod } = req.body;

    if (!project || !amount) {
      return res.status(400).json({ status: "fail", message: "Please provide project and amount" });
    }
    if (Number(amount) < 10) {
      return res.status(400).json({ status: "fail", message: "Minimum donation is KES 10" });
    }
    if (paymentMethod === "mpesa" && !phone) {
      return res.status(400).json({ status: "fail", message: "Please provide phone number for M-Pesa" });
    }

    const projectDoc = await Project.findById(project);
    if (!projectDoc || projectDoc.status !== "active") {
      return res.status(404).json({ status: "fail", message: "Project not available for donation" });
    }
  ...
  try {
    ...
    if (event === "charge.success") {
      ...
      if (proj) {
        ...
        if (proj.raisedAmount >= proj.goalAmount) proj.status = "completed";
        await proj.save();
      }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (status === "success") {
      const donation = await Donation.findOne({ reference: req.params.reference });
    ...
    try {
      ...
      res.status(200).json({ status: "success", data: { paymentStatus: donation.status } });
    } catch (err) {
...
exports.calculateTotalDonations = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: "success", data: { ...data, currency: "KES" } });
  } catch (error) {
...
exports.monthlyDonations = async (req, res) => {
  try {
    ...
    const projectDoc = await Project.findById(project);
    if (!projectDoc) return res.status(404).json({ status: "fail", message: "Project not found" });

    const donation = await Donation.create({ donor, amount, paymentMethod, project, status: "success" });
    projectDoc.raisedAmount += Number(amount);
    if (projectDoc.raisedAmount >= projectDoc.goalAmount) projectDoc.status = "completed";
...
exports.getMonthlyStats = async (req, res) => {
  try {
    ...
    res.status(200).json(stats.map((i) => ({ month: months[i._id - 1], totalAmount: i.totalAmount })));
  } catch (error) {
...
exports.donateToProject = async (req, res) => {
  try {
    ...
    if (!amount || isNaN(amount) || Number(amount) < 1) {
      return res.status(400).json({ status: "fail", message: "Please provide a valid amount" });
    }
    const project = await Project.findById(req.params.id);
    if (!project) return res.status(404).json({ status: "fail", message: "Project not found" });
    if (project.status === "completed") {
      return res.status(400).json({ status: "fail", message: "This project has already reached its goal" });
    }
    project.raisedAmount += Number(amount);
    const completed = project.raisedAmount >= project.goalAmount;
    if (completed) { project.status = "completed"; project.raisedAmount = project.goalAmount; }
    await project.save();
    res.status(200).json({
      status: "success",
      message: completed ? "Goal reached! Project completed 🎉" : "Donation successful",
      data: {
        project: {
          _id: project._id,
          raisedAmount: project.raisedAmount,
          goalAmount: project.goalAmount,
          progressPercentage: Math.min(Math.round((project.raisedAmount / project.goalAmount) * 100), 100),
