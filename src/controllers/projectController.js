    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
          ...
          ${
            project.image
              ? `<img src="${project.image}" alt="Cover Image" style="width:100%;border-radius:8px;margin-bottom:16px;" />`
              : ""
          }
          <h2 style="color:#444;">${project.title}</h2>
          <p style="color:#666;font-size:15px;line-height:1.6;">${project.description.substring(
            0,
            200
          )}...</p>
...
exports.countProjects = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", data: { countCompleted, countActive } });
  } catch (err) {
    res.status(500).json({ status: "error", message: err.message });
  }
};
